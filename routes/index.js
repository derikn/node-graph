...
/* GET drive page displaying all the files, and users to work with */
    ...
    requestUtil.getDriveData(
      ...
      function(data){
        ...
        res.render(
          'drive', 
          {
            drive_data: JSON.stringify(data)
          }
        )
      }
    )
...
/* GET script run page, sending a series of post share requests to change permissions
on the drive files */
router.get('/drive/share', function(req,res){
  //check for the goddamn token
  if (req.cookies.REFRESH_TOKEN_CACHE_KEY === undefined){
    res.redirect('login');
  } else {
    requestUtil.getDriveData(
